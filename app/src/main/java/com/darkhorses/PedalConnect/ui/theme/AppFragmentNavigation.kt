...
fun AppNavigator(paddingValues: PaddingValues) {
    ...
    NavHost(navController, startDestination = startDest) {
        ...
        composable(
            ...
            VerifyEmailScreen(navController, userName)
        }
        ...
        composable(
            "plan_history/{userName}",
            arguments = listOf(navArgument("userName") { type = NavType.StringType })
        ) { backStackEntry ->
            val userName = backStackEntry.arguments?.getString("userName") ?: "Rider"
            PlanHistoryScreen(navController, userName)
        }
        ...
        composable(
            "create_event/{userName}",
            arguments = listOf(navArgument("userName") { type = NavType.StringType })
        ) { backStackEntry ->
            val userName = backStackEntry.arguments?.getString("userName") ?: "Rider"
            CreateEventScreen(navController, userName)
        }
        ...
        composable(
            ...
            // Side effects (prefs write + navigate) must run exactly once when this
            // destination is entered — doing them directly in the composable body
            // re-fires them on every recomposition, causing repeated navigate() calls
            // that present as a brief freeze/loop before the backstack settles.
